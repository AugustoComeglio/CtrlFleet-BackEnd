    ...
    class FleetsController < Api::BaseController
      ...
      def create
        ...
        if @fleet.save
          ...
          render json: { message: @fleet.errors.full_messages.join(', ') }, status: :not_found
        end
      end
      ...
      def vehicles
        render json: { data: @fleet.vehicles.as_json(only: %i[id license_plate], methods: %i[brand_name model_name]) }, status: :ok
      end
