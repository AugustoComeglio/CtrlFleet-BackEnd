    ...
    class UnitTypesController < Api::BaseController
      ...
      def create
        ...
        if @unit_type.save
          ...
          render json: { message: @unit_type.errors.full_messages.join(', ') }, status: :not_found
        end
      end
      ...
      def units
        if @unit_type.units.present?
          render json: { data: @unit_type.units.as_json(only: %i[id name]) }, status: :ok
        else
          render json: { message: 'No existen unidades asociadas' }, status: :not_found
        end
      end
