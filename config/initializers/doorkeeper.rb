...
Doorkeeper.configure do
  ...
  # TODO: leer https://github.com/doorkeeper-gem/doorkeeper/issues/669#issuecomment-183771559
  #       investigar acerca de doorkeeper application https://doorkeeper.gitbook.io/guides/ruby-on-rails/api-mode
  skip_client_authentication_for_password_grant true

  resource_owner_from_credentials do
    user = User.find_for_database_authentication(email: params[:username])
    if user&.type&.enable_login
      user if user&.valid_for_authentication? { user.valid_password?(params[:password]) }
    end
  end

  # resource_owner_from_credentials do
  #   app = Doorkeeper::Application.by_uid('qI_1hx8kUupt8lwHO7vZ5_Ze19OuRebOteo_eumlxuo')
  #   if app
  #     user = User.find_for_database_authentication(email: params[:username])
  #     user if user&.valid_for_authentication? { user.valid_password?(params[:password]) }
  #   end
  ...
  admin_authenticator do |routes|
    # current_user&.role?('admin') || redirect_to(routes.root_url)
    current_user&.type&.enable_login || redirect_to(routes.root_url)
  end
  ...
  access_token_class 'CtrlfleetAccessToken'
  access_grant_class 'CtrlfleetAccessGrant'
  application_class 'CtrlfleetApplication'
...
Rails.application.config.to_prepare do
  Doorkeeper::OAuth::TokenResponse.send :prepend, CtrlfleetTokenResponse
end
